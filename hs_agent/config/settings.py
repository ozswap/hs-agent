...
class HSAgentSettings(BaseSettings):
    ...
    google_api_key: str | None = Field(
        None, description="Google API key for Vertex AI/Gemini models", env="GOOGLE_API_KEY"
    )
    ...
    fallback_model_name: str = Field(
        "gemini-2.5-flash", description="Fallback model if default fails", env="FALLBACK_MODEL_NAME"
    )
    ...
    default_top_k: int = Field(
        10, description="Default number of candidates to consider", env="DEFAULT_TOP_K", ge=1, le=50
    )
    ...
    hs_codes_file: str = Field(
        "hs_codes_all.csv", description="HS codes CSV file name", env="HS_CODES_FILE"
    )
    ...
    # === API Configuration ===
    api_host: str = Field("0.0.0.0", description="API server host", env="API_HOST")

    api_port: int = Field(9999, description="API server port", env="API_PORT", ge=1, le=65535)

    api_workers: int = Field(1, description="Number of API workers", env="API_WORKERS", ge=1)
    ...
    api_timeout: int = Field(
        300, description="API client timeout in seconds", env="API_TIMEOUT", ge=10
    )

    use_local_mode: bool = Field(
        False, description="Use local agents instead of API client", env="USE_LOCAL_MODE"
    )

    # === Performance Configuration ===
    enable_caching: bool = Field(
        True, description="Enable data caching for performance", env="ENABLE_CACHING"
    )

    cache_ttl_seconds: int = Field(
        3600, description="Cache TTL in seconds", env="CACHE_TTL_SECONDS", ge=60
    )
    ...
    request_timeout_seconds: int = Field(
        300, description="Request timeout in seconds", env="REQUEST_TIMEOUT_SECONDS", ge=30
    )
    ...
    enable_logfire_logging: bool = Field(
        True, description="Enable Logfire observability logging", env="ENABLE_LOGFIRE"
    )
    ...
    logfire_service_name: str = Field(
        "hs-agent", description="Service name shown in Logfire", env="LOGFIRE_SERVICE_NAME"
    )
    ...
    log_file: Path | None = Field(
        None, description="Log file path (if None, logs to console)", env="LOG_FILE"
    )

    # === Development Configuration ===
    debug_mode: bool = Field(False, description="Enable debug mode", env="DEBUG_MODE")

    development_mode: bool = Field(
        False, description="Enable development mode features", env="DEVELOPMENT_MODE"
    )
    ...
    def google_api_enabled(self) -> bool:
        ...
        return (
            ...
            and self.google_api_key != "your_google_api_key_here"  # pragma: allowlist secret
        )
